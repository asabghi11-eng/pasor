...
(function () {
  ...
  const CSS = `
  .hks-fab {
    position: fixed; bottom: 124px; inset-inline-end: 18px; z-index: 9998;
  ...
  .hks-head { display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; border-bottom: 1px solid #17323d; background: linear-gradient(135deg, #12242c, #0d1a20); }
  .hks-head h2 { margin: 0; font-size: 16px; }
  .hks-close { cursor: pointer; font-size: 20px; line-height: 1; color: #a9d8e8; background: none; border: none; }
  .hks-tabs { display: flex; border-bottom: 1px solid #17323d; }
  .hks-tab { flex: 1; text-align: center; padding: 10px 4px; cursor: pointer; font-size: 13px; color: #7aa6b5; border-bottom: 2px solid transparent; }
  .hks-tab.hks-active { color: #3fb6d6; border-bottom-color: #3fb6d6; }
  .hks-body { padding: 12px 16px; overflow-y: auto; flex: 1; }
  .hks-panel { display: none; }
  .hks-panel.hks-active { display: block; }

  .hks-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 12px; }
  .hks-stat { background: #12242c; border-radius: 10px; padding: 10px; text-align: center; }
  .hks-stat .hks-num { font-size: 18px; font-weight: 700; color: #3fb6d6; }
  .hks-stat .hks-lbl { font-size: 11px; color: #7aa6b5; margin-top: 2px; }

  .hks-hint-btn { width: 100%; border: none; border-radius: 8px; background: #3fb6d6; color: #0d1a20; font-weight: 700; padding: 10px 12px; cursor: pointer; margin-bottom: 4px; }
  .hks-hint-btn:disabled { background: #1c3a44; color: #5c8492; cursor: not-allowed; }

  .hks-card { background: #12242c; border-radius: 10px; padding: 10px; margin-bottom: 10px; }
  .hks-card .hks-name { font-weight: 700; font-size: 14px; display: flex; justify-content: space-between; }
  .hks-card .hks-meta { font-size: 12px; color: #7aa6b5; margin-top: 2px; }
  .hks-card button { margin-top: 8px; border: none; border-radius: 8px; background: #3fb6d6; color: #0d1a20; font-weight: 700; padding: 7px 12px; cursor: pointer; }
  .hks-badge { border-radius: 999px; padding: 2px 8px; font-size: 11px; font-weight: 700; }
  .hks-badge.hks-win { background: rgba(63,214,138,.18); color: #3fd68a; }
  .hks-badge.hks-loss { background: rgba(230,90,90,.18); color: #e65a5a; }

  .hks-row { display: flex; align-items: center; justify-content: space-between; padding: 7px 4px; border-bottom: 1px solid #17323d; font-size: 13px; }
  .hks-row .hks-rank { width: 22px; color: #7aa6b5; }
  .hks-row.hks-me { background: rgba(63,182,214,.12); border-radius: 6px; }
  .hks-note { color: #7aa6b5; font-size: 12px; text-align: center; padding: 20px 0; }

  .hks-tips { margin: 10px 0 0; padding-inline-start: 18px; font-size: 12px; color: #a9d8e8; }
  .hks-tips li { margin-bottom: 4px; }

  .hks-hand { background: #10202a; border-radius: 8px; padding: 8px 10px; margin-bottom: 8px; }
  .hks-hand .hks-hand-head { font-size: 12px; color: #7aa6b5; margin-bottom: 4px; }
  .hks-trick { font-size: 13px; padding: 2px 0; display: flex; justify-content: space-between; }
  .hks-back { background: none; border: none; color: #3fb6d6; cursor: pointer; font-size: 13px; padding: 0 0 10px; }
  ...
  class StatsPanel {
    ...
    _build() {
      ...
      this.fab = el("div", "hks-fab", "📊 " + t('stt_fab_label','آمار'));
      ...
      modal.innerHTML = `
        ...
        <div class="hks-tabs">
          <button class="hks-tab hks-active" data-tab="stats">${t('stt_tab_stats','آمار من')}</button>
          <button class="hks-tab" data-tab="history">${t('stt_tab_history','تاریخچه / ریپلی')}</button>
          <button class="hks-tab" data-tab="leaderboard">${t('stt_tab_leaderboard','لیدربورد')}</button>
        </div>
        <div class="hks-body">
          <div class="hks-panel hks-active" data-panel="stats"></div>
          <div class="hks-panel" data-panel="history"></div>
          <div class="hks-panel" data-panel="leaderboard"></div>
        </div>
      `;
      ...
      modal.querySelectorAll(".hks-tab").forEach((tab) => tab.addEventListener("click", () => this._switchTab(tab.dataset.tab)));
    ...
    open() { this.overlay.classList.add("hks-open"); }
    close() { this.overlay.classList.remove("hks-open"); }
    ...
    _toast(text) {
      ...
      this._toastTimer = setTimeout(() => this.toastEl.classList.remove("hks-show"), 4200);
    }
    ...
    _renderStats() {
      ...
      this._hintBtn = el("button", "hks-hint-btn", "💡 " + t('stt_hint_btn','پیشنهاد حرکت (فقط سر نوبت خودت)'));
      ...
      if (!this.stats) {
        panel.appendChild(el("div", "hks-note", t('stt_no_stats','هنوز آماری ثبت نشده — یک مسابقه بازی کن!')));
        return;
      }
      const s = this.stats;
      const grid = el("div", "hks-grid");
      const items = [
      ...
      items.forEach(([num, lbl]) => {
        const box = el("div", "hks-stat");
        box.innerHTML = `<div class="hks-num">${num}</div><div class="hks-lbl">${lbl}</div>`;
    ...
    _renderHistory() {
      ...
      if (!this.matches.length) {
        panel.appendChild(el("div", "hks-note", t('stt_no_matches','هنوز مسابقه‌ای تموم نشده.')));
        return;
      }
      this.matches.forEach((m) => {
        const card = el("div", "hks-card");
        const badge = m.won ? `<span class="hks-badge hks-win">${t('stt_win_badge','برد')}</span>` : `<span class="hks-badge hks-loss">${t('stt_loss_badge','باخت')}</span>`;
        card.innerHTML = `
          <div class="hks-name">${fmtDate(m.ts)} ${badge}</div>
          <div class="hks-meta">${t('stt_hands_score','امتیاز دست‌ها:')} ${m.roundsWon.A} - ${m.roundsWon.B}${m.analysis ? ` · ${t('stt_optimal_rate','بهینه:')} ${m.analysis.optimalRate}%` : ""}</div>
        `;
    ...
    _renderReplay(panel) {
      const back = el("button", "hks-back", "← " + t('stt_back_to_history','بازگشت به تاریخچه'));
      back.addEventListener("click", () => { this.replayView = null; this._renderHistory(); });
      panel.appendChild(back);

      const match = this.replayView;
      if (!match) {
        panel.appendChild(el("div", "hks-note", t('stt_replay_not_found','این ریپلی پیدا نشد.')));
        return;
      }

      if (match.analysis && match.analysis.tips && match.analysis.tips.length) {
        const tipsBox = el("div", "hks-card");
        tipsBox.innerHTML = `<div class="hks-name">🧠 ${t('stt_smart_analysis','تحلیل هوشمند (بهینه: {rate}%)').replace('{rate}', match.analysis.optimalRate)}</div>`;
        const ul = el("ul", "hks-tips");
      ...
      (match.hands || []).forEach((h) => {
        const box = el("div", "hks-hand");
        box.innerHTML = `<div class="hks-hand-head">${t('stt_hand_label','دست')} ${h.handNumber} — ${t('stt_hakem_label','حاکم:')} ${seatLabel(h.hakem)} — ${t('stt_trump_label','حکم:')} ${SUIT_SYMBOL[h.trump] || h.trump} — ${t('stt_result_label','نتیجه:')} ${h.tricksWon.A}-${h.tricksWon.B}</div>`;
        (h.tricks || []).forEach((trick, i) => {
          const row = el("div", "hks-trick");
    ...
    _renderLeaderboard() {
      ...
      if (!top.length) {
        panel.appendChild(el("div", "hks-note", t('stt_no_ranked','هنوز کسی مسابقه‌ای تموم نکرده.')));
        return;
      }
      top.forEach((pl) => {
        const row = el("div", "hks-row" + (pl.playerId === this.myPlayerId ? " hks-me" : ""));
        row.innerHTML = `<span><span class="hks-rank">${pl.position}.</span>${pl.name}</span><span>${t('stt_wins_pct','{wins} برد ({rate}%)').replace('{wins}', pl.matchesWon).replace('{rate}', pl.winRate)}</span>`;
        panel.appendChild(row);
      });
      const me = this.leaderboard.me;
      if (me && me.position > top.length) {
        const sep = el("div", "hks-note", "···");
        panel.appendChild(sep);
        const row = el("div", "hks-row hks-me");
        row.innerHTML = `<span><span class="hks-rank">${me.position}.</span>${me.name}</span><span>${t('stt_wins_pct','{wins} برد ({rate}%)').replace('{wins}', me.matchesWon).replace('{rate}', me.winRate)}</span>`;
        panel.appendChild(row);
      }
    }
  }
